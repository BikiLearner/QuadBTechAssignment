...
class _SearchScreenState extends State<SearchScreen> {
  List<movie_entity.Movie> searchResults = [];
  bool isLoading = true;
  String? errorMessage;
  ...
  Future<void> searchMovies(String query) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          errorMessage = 'Failed to search movies. Status Code: ${response.statusCode}';
          isLoading = false;
        });
      }
    } catch (e) {
      setState(() {
        errorMessage = 'An error occurred while searching movies: $e';
        isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (index) {
          if (index == 0) {
            Navigator.pushNamed(context, '/home'); // Assuming route is set up for SearchScreen
          }
        },
        items: [
